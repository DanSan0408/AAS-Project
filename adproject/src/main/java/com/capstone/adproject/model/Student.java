...
@Filter(name = "courseScopeFilter", condition = "({alias}.course_id = :activeCourseId OR {alias}.course_id IS NULL OR EXISTS (SELECT 1 FROM student_course_assignment sca WHERE sca.student_id = {alias}.id AND sca.course_id = :activeCourseId) OR EXISTS (SELECT 1 FROM project_group pg WHERE pg.id = {alias}.group_id AND pg.course_id = :activeCourseId))")
@Entity
