...
@Filter(name = "courseScopeFilter", condition = "(course_id = :activeCourseId OR course_id IS NULL OR EXISTS (SELECT 1 FROM student_course_assignment sca WHERE sca.student_id = id AND sca.course_id = :activeCourseId) OR EXISTS (SELECT 1 FROM project_group pg WHERE pg.id = group_id AND pg.course_id = :activeCourseId))")
@Entity
